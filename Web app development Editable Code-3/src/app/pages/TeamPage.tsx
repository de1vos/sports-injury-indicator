        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            <tr className="border-b border-[rgba(0,0,0,0.06)]">
              ...
              <td className="py-3 px-2 text-center font-mono text-[#1A1A2E]">{player.foulsAgainst}</td>
            ...
            {['2024/25', '2023/24'].map(season => (
              <tr key={season} className="border-b border-[rgba(0,0,0,0.06)] last:border-0">
                ...
                {Array(5).fill(null).map((_, i) => (
                  <td key={i} className="py-3 px-2 text-center font-mono text-[#6B7280]">-</td>
                ))}
              </tr>
            ))}
          </tbody>
            ...
            <tbody>
              {player.injuryHistory.map((injury, index) => (
                <tr key={index} className="border-b border-[rgba(0,0,0,0.06)] last:border-0">
                  <td className="py-3 px-2 text-[#1A1A2E]">{injury.diagnosis}</td>
                  <td className="py-3 px-2 text-[#6B7280]">{injury.region}</td>
                  <td className="py-3 px-2 text-[#1A1A2E] whitespace-nowrap">{injury.from}</td>
                  <td className="py-3 px-2 text-[#1A1A2E] whitespace-nowrap">{injury.until}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
      ...
      <div className="flex items-center justify-center gap-1 mb-12">
        ...
        {SORT_OPTIONS.map((opt, i) => (
          <>
            {i > 0 && <span key={`sep-${i}`} className="text-[#6B7280]">·</span>}
            <button
              key={opt.value}
              onClick={() => setSortBy(opt.value)}
              className={`px-3 py-1 text-sm rounded-full transition-colors ${
                sortBy === opt.value ? 'bg-[#1A56DB] text-white' : 'text-[#6B7280] hover:bg-[#F5F6FA]'
              }`}
            >
              {opt.label}
            </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-[rgba(0,0,0,0.06)] p-6">
            ...
            <div className="space-y-4">
              {[
                { label: 'Games Played', value: currentPlayer.gamesPlayed, max: 38, color: '#F59E0B', display: String(currentPlayer.gamesPlayed) },
                { label: 'Minutes Played', value: currentPlayer.minutesPlayed, max: 3000, color: '#1A56DB', display: currentPlayer.minutesPlayed.toLocaleString() },
                { label: 'Fouls Against', value: currentPlayer.foulsAgainst, max: 4, color: '#DC2626', display: String(currentPlayer.foulsAgainst) },
                { label: 'Aerial Duels', value: 0, max: 1, color: '#0D9488', display: '-' },
                { label: 'Tackles', value: 0, max: 1, color: '#8B5CF6', display: '-' },
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-[rgba(0,0,0,0.06)] p-6">
            ...
            <div className="space-y-4">
              {[
                ...
                { label: 'Matches Missed', value: Math.round(currentPlayer.minutesMissed / 90) },
                { label: 'Weeks Missed', value: Math.round(currentPlayer.daysSinceLastInjury / 7) },
                { label: 'Days Since Last Injury', value: currentPlayer.daysSinceLastInjury },
                { label: 'Matches Played Per Week', value: currentPlayer.matchDensity },
              ].map(({ label, value, red }) => (
                <div key={label} className="flex items-center justify-between py-4 px-6 bg-[#F5F6FA] rounded-xl">
                  ...
                  <span
                    ...
                    style={{ fontFamily: 'var(--font-mono)', color: red ? '#DC2626' : '#1A1A2E' }}
                  >
                    {value}
                  </span>
                </div>
