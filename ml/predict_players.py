...
For every active 2025/26 PL player:
  ...
  - Risk trend over last 38 gameweeks (dict: GW1–GW38, current season first, prev season backfill)
  - Top 3 risk factors (SHAP if available, else rule-based)
  - All season stats (array, newest first)
  - Injury summary and full injury history
  - Next match

Outputs:
  output/predictions.json  — {current_gameweek, current_season, generated_at, players}
...
FIXTURES_PREV_FILE = Path(FIXTURES_FILE).parent / f"fixtures_{CURRENT_SEASON - 1}.json"
...
FACTOR_TEMPLATES = {
    ...
    "age_squared":                lambda v: "Age-related physical decline",
    "upcoming_matches_next_14d":  lambda v: f"Fixture congestion: {int(v)} upcoming matches in 14d",
    "same_region_recurrence_flag":lambda v: "High recurrence risk in same body region",
    "is_acwr_danger_zone":        lambda v: "Critical workload spike (ACWR > 1.5)",
    "position_encoded":           lambda v: "Positional biomechanical demands",
...
FACTOR_RULES = [
    ...
    ("is_acwr_danger_zone",        lambda v: v == 1,      "is_acwr_danger_zone"),
    ("upcoming_matches_next_14d",  lambda v: v >= 3,      "upcoming_matches_next_14d"),
    ("same_region_recurrence_flag",lambda v: v == 1,      "same_region_recurrence_flag"),
    ("age_squared",                lambda v: v >= 900,    "age_squared"),
...
def main():
    ...
    for player_id in active_ids:
        ...
        # ── Risk trend ────────────────────────────────────────────────────────
        # 38 slots labeled GW1–GW38.
        # Current season fixtures fill GW1 onwards; previous season backfills tail.
        ...
        n_prev           = max(0, TREND_MATCHES - len(curr_fixtures))
        ordered_fixtures = prev_fixtures[-n_prev:] + curr_fixtures
